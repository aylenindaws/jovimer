    ...
    def default_get(self, default_fields):
        ...
        for record in data.order_line:
            update.append((0, 0, {
                ...
                'tipouom': record.tipouom,
                'bultos': record.bultos,
                'variedad': record.variedad,
                'confeccion': record.confeccion,
                'calibre': record.calibre,
                'marca': record.marca,
                'envase': record.purchase_price,
                'account_analytic_id': data.analytic_account_id,
        ...
        for supplier in supplier_list:
            ...
            for order_line in purchase_order_lines:
                value.append([0, 0, {
                    ...
                    'tipouom': order_line.tipouom,
                    'bultos': order_line.bultos,
                    'variedad': order_line.variedad,
                    'confeccion': order_line.confeccion,
                    'calibre': order_line.calibre,
                    'marca': order_line.marca,
                    'envase': order_line.purchase_price,
                    'account_analytic_id': order_line.account_analytic_id,
                }])
            res.create({
